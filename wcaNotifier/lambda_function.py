...
def lambda_handler(event, context):
    try:
        ...
        for recip, comps in emailNotif.items():
            ...
            ses.send_email(Destination={"ToAddresses": [recip]}, Content={"Simple": {"Body": {"Html": {"Data": email}}, "Subject": {"Data": "WCA Notification"}, "Headers": [{"Name": "List-Unsubscribe", "Value": f"<https://m4q4s5pxsarghztkazrode4mdq0dtfli.lambda-url.us-east-2.on.aws/?uuid={emailUUIDs[recip]}>"}, {"Name": "List-Unsubscribe-Post", "Value": "List-Unsubscribe=One-Click"}]}}, FromEmailAddress="WCA Notifier <updates@wcanotifier.com>")
            sleep(0.1)
    except:
        ses.send_email(Destination={"ToAddresses": ["updates@wcanotifier.com"]}, Content={"Simple": {"Body": {"Text": {"Data": format_exc()}}, "Subject": {"Data": "WCA Notification Failed!"}}}, FromEmailAddress="WCA Notifier <updates@wcanotifier.com>")